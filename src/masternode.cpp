...
{
    if(nHeight > 1200000) return 30000 * COIN;
...
{
    if(nHeight > 1600000) return    0.5 * COIN;
    if(nHeight > 1450000) return      1 * COIN;
    if(nHeight > 1200000) return    1.5 * COIN;
    if(nHeight >  800000) return      2 * COIN;
    if(nHeight >  500000) return    1.5 * COIN;
    if(nHeight >  300000) return      4 * COIN;
    if(nHeight >  100000) return      5 * COIN;
    if(nHeight >   80000) return     15 * COIN;
    if(nHeight >   60000) return    5.6 * COIN;
    if(nHeight >   50000) return    8.8 * COIN;
    if(nHeight >   40000) return    4.4 * COIN;
    if(nHeight >   30000) return     10 * COIN;
    if(nHeight >   20000) return     12 * COIN;
    if(nHeight >    5000) return     16 * COIN;
    if(nHeight >     200) return     20 * COIN;
    if(nHeight >       1) return  0.001 * COIN;
...
CAmount CMasternode::GetMasternodePayment(int nHeight)
{
    ...
    if(nHeight > 1000000) return GetBlockValue(nHeight) * 90 / 100;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                   // block for 1000 JHL tx -> 1 confirmation

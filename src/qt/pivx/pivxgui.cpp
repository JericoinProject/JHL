...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Jericoin client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Jericoin"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Jericoin"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
